...
| The SDKs — [`clients/typescript-sdk`](clients/typescript-sdk), [`clients/go-sdk`](clients/go-sdk), [`crates/fc-sdk`](crates/fc-sdk) (Rust) | [Apache-2.0](https://www.apache.org/licenses/LICENSE-2.0) | `LICENSE` in each SDK directory |
| The Laravel SDK — [`clients/laravel-sdk`](clients/laravel-sdk) | [MIT](https://opensource.org/license/mit) | [`clients/laravel-sdk/LICENSE`](clients/laravel-sdk/LICENSE) |
...
in customer applications, so they keep the permissive licences they are
published under (TypeScript Apache-2.0, Laravel MIT; the Go and Rust SDKs
follow the TypeScript SDK). Your application that uses them stays yours under
whatever license you choose. Using an SDK does not make your application AGPL.
The function guest crates are MPL-2.0, a file-level copyleft: changes to their
own files must be shared, but a function built with them is not affected.
