...
- **Login-backoff dormant-identifier default (ruled 2026-09-03, option a):** an identifier whose last success lies beyond the bounded lookup window is treated as never-succeeded — the standard failure-counting window applies in full. Dormancy never weakens the lockout; no full-history reach-back (the retracted design's enumeration-triggered full scan stays dead). Pinned by test in the Go implementation.
- **SQS visibility × redrive interaction (closed 2026-09-03, not applicable):** the production queues have NO redrive policy — no maxReceiveCount, no DLQ — so a long-but-succeeding delivery can never be swept; redelivery churn on the small slow fraction is accepted (dedup absorbs it). Consistent with A-02: broker retention is the sole terminal condition. Revisit only if a redrive policy is ever introduced — then visibility × maxReceiveCount must exceed the 15-min delivery contract with margin, and the router should gain a startup CONFIGURATION check.
