...
| `PLATFORM_ENABLED` | `FC_PLATFORM_ENABLED` alias, default true. Serves the API and the embedded SPA; runs the purger and the A-01 reaper | Same alias | Same. Toggles parse as Go's `envBool` (`fc_common::config::env_first_bool_go`) (`1/true/yes/on`, `0/false/no/off`, any case, else the default; a set primary name wins over its alias). The lapsed-secret purge and the rate-limit prune now run only where the platform runs |
...
| `MESSAGE_ROUTER_ENABLED` | Alias of `FC_ROUTER_ENABLED`, default false | Same | Same. Router wiring and the router-only role (no database unless a DB subsystem runs) are `feat/router-env`'s, see `docs/parity/router-env-vs-go.md` |
