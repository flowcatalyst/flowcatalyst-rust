...
| D2 | BLOCK_ON_ERROR, siblings behind a terminally failed head | ACKed, and reported to the platform through the settled hook (`pool.go:939`, `ackBuffered`) | The whole buffer is taken and **nacked** | Ledger A-01: the platform half (settled hook plus reaper) does not exist in this port. ACKing without it would strand the job rows. |
