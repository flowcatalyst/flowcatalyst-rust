...
| A19 | **Password leak in audit logs (fixed going forward, `3a09eb7d`).** `CreateUserCommand.password` and `ResetPasswordCommand.new_password` were serialised into `aud_logs.operation_json`. The prod audit counted about 290 affected rows. | `principal/operations/{create,reset_password}.rs` | Existing rows need a targeted `jsonb` scrub, planned once fc-audit-3 identifies the keys. Owner to run it. |
