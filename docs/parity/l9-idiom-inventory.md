...
| A18 ✔ | **Every service account has ANCHOR scope** (`principal/entity.rs:259` `Principal::new_service`; Go does the same in `principal/entity.go:195`), and about 112 handlers across 19 files are guarded only by `require_anchor`. So any service-account token, including an application's SDK credential, passes those guards: principal management and password resets, OAuth clients, identity providers, roles, applications, clients, audit logs, platform config writes and more. | `grep -rn require_anchor crates/fc-platform/src` | **Owner decision:** stop giving service accounts anchor scope, or make `require_anchor` require a user principal / an explicit admin permission. Needs matching Go changes. |
