...
| Platform P5 | A13 secrets at rest + backfill | done, including OAuth client secrets as Go-compatible `hashed:v1:` |
...
| A18 ✔ | **Fixed (Rust-specific; `75990d46`, `bf1182e1`, `90a44764`).** Rust's create-service-account endpoint silently dropped the `scope` chosen in the UI, and `Principal::new_service` forced ANCHOR, so every Rust-created service account passed about 112 `require_anchor`-only guards. Go and Java honour the chosen scope. Rust now takes `scope` (strict), checks it against client access, and adopts Go's `all_applications` flag (migration 031 mirrors Go's 034). New service accounts get the flag false and no applications (owner ruling), and a detail-page toggle controls it. | `service_account/`, `principal/entity.rs`, `shared/authorization_service.rs` | Existing rows are unchanged. |
| A19 ✔ | **Password and secret leak in audit logs: fixed at the source plus a sweep.** First `3a09eb7d` (`skip_serializing`), then the audit-redaction spec from the Java repo (`docs/spec/audit-redaction.md` there): one rule in `fc-common/src/audit_redaction.rs`, shared vectors (`docs/spec/audit-redaction-vectors.json`, byte-identical copies checked by a test), redaction on every UoW audit write, the ingest backstop, and the same rule in the fc-sdk, TS and Laravel SDKs. Temporary `POST /bff/audit-logs/redact-existing` plus a dashboard **Redact** card (`194de0fb`..`b36bc522`). The prod audit found about 290 rows with plaintext `password`/`newPassword`; the Go-style `Principal/*Command` rows were still being written on 2026-09-16. | `fc-common`, `usecase/`, `audit/`, SDKs, `frontend/src/pages/DashboardPage.vue` | Owner: run Redact; rotate the exposed passwords; Go's sink needs the same rule before cutover. |
...
## Open questions raised by A15 and P5b (2026-09-24)

- **Stored `all_applications` flag.** Go stores `iam_principals.all_applications` (default true; provisioning sets it false and adds an access row). Rust treats "no `application_id`" as all applications. If Go-provisioned service accounts have a NULL `application_id`, Rust's A15 check won't confine them. fc-audit-3 checks this. The fix is probably to adopt Go's flag.
- **Rust provisioning never assigns the `platform:application-service` role** (Go does, and `docs/developers/processes.md` says it's automatic). A Rust-provisioned service account passes the app check but gets 403 on the SDK permission checks until an admin assigns the role.
- **Read routes with no permission check:** `/api/roles/by-application/{applicationId}`, `/api/applications/by-id/{id}/roles`, and the config reads. Also `client/api.rs` `/{id}/applications/{applicationId}/…` isn't app-scoped, and platform-scoped scheduled-jobs sync accepts `is_anchor()` (see A18).
- **`previous_secret_ref`:** Rust has no rotation-overlap window, so during Go's overlap Rust rejects a rotated client's previous secret.
- **Out-of-scope app:** Rust answers 404, Go's sdksync answers 403 (the owner rulings ask for 404).

## Notes from the service-account work (2026-09-24)

- **Deviation from Go, to confirm:** a service account bound to an app (`application_id` set) reaches only that
  app plus its grants, even when `all_applications` is true. Otherwise the column default `true` would widen
  every account provisioned before the flag existed from its own app to every app.
- **Possible Go bug, for the Go/Java side:** Go stores the requested service-account scope unchecked and derives
  the token tier from client links (no clients gives ANCHOR). A CLIENT-scope account created with no clients
  therefore becomes ANCHOR. Rust rejects that combination with a 400.
- **Pre-existing, unrelated Docker test failures** (they also fail at the L9 branch point 15fd4310):
