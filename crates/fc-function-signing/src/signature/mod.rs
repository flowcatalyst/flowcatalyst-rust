...
//!
//! **Why a port, not `sigstore-rs`.** The platform and the host must accept
//! and reject exactly the bundles Java does, with the same reason codes
...
#[derive(Clone)]
pub enum Signatures {
    /// Every artifact must carry a bundle that verifies. At publish, its
    /// signer must be one the owner's policy permits; on a host, the one the
    /// platform recorded at publish.
