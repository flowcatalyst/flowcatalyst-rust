...
pub enum BadgeVariant {
    ...
    Destructive,
    /// FlowCatalyst: PrimeVue's `success` Tag (active, enabled).
    Success,
    /// FlowCatalyst: PrimeVue's `info` Tag.
    Info,
    /// FlowCatalyst: PrimeVue's `warn` Tag.
    Warn,
}
...
impl BadgeVariant {
    ...
    fn classes(self) -> StaticClass {
        match self {
            ...
            Self::Success => class!("border-transparent bg-success text-success-foreground"),
            Self::Info => class!("border-transparent bg-info text-info-foreground"),
            Self::Warn => class!("border-transparent bg-warning text-warning-foreground"),
        }
    }
}
...
const BASE: StaticClass = class!(
    "inline-flex w-fit shrink-0 items-center justify-center gap-1 rounded-md \
     border px-2 py-0.5 text-xs font-bold whitespace-nowrap",
);
