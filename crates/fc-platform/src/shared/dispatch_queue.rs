...
/// The tenant of platform-owned (client-less) work.
pub const TENANT_PLATFORM: &str = "platform";
/// SQS's queue-name limit, `.fifo` included.
pub const SQS_MAX_NAME_LENGTH: usize = 80;
/// The default pool's code, before tenant composition.
pub const DEFAULT_POOL_CODE: &str = "DEFAULT-POOL";
/// The default pool's composed suffix (`platform-DEFAULT-POOL`, …).
pub const DEFAULT_POOL_SUFFIX: &str = "-DEFAULT-POOL";

/// A dispatch queue's priority segment (never sanitised).
...
impl Priority {
    ...
    /// Go `ForPublishing`: `HIGH_PRIORITY` (any case, trimmed) is high
    /// priority; anything else, and no value, is the default queue.
    pub fn for_publishing(queue: Option<&str>) -> Self {
        match queue {
            Some(q) if q.trim().eq_ignore_ascii_case("HIGH_PRIORITY") => Priority::HighPriority,
            _ => Priority::Default,
        }
    }
}
...
pub enum NameError {
    /// Go `ErrTenantRequired`.
    TenantRequired,
    /// Go `NameTooLongError`.
    TooLong { tenant: String, composed: String },
}

impl fmt::Display for NameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            NameError::TenantRequired => write!(f, "dispatch queue tenant is required"),
            NameError::TooLong { tenant, composed } => write!(
                f,
                "dispatch queue name {:?} ({} chars) exceeds SQS's {}-character limit for tenant {:?}",
                composed,
                composed.len(),
                SQS_MAX_NAME_LENGTH,
                tenant
...
/// Go `sanitiseTenant`: `_`, `.` and space become `-`.
pub fn sanitise_tenant(tenant: &str) -> String {
    tenant.replace(['_', '.', ' '], "-")
}
...
/// Go `ComposePoolCode`: `{tenant}-{poolCode}`.
pub fn compose_pool_code(pool_code: &str, client_identifier: Option<&str>) -> String {
    format!("{}-{}", tenant_for(client_identifier), pool_code)
}

/// Go `ComposeName`: `[{prefix}-]{tenant}-{priority}`, `.fifo` on SQS.
pub fn compose_name(
    prefix: &str,
    tenant: &str,
    priority: Priority,
    ...
    if tenant.trim().is_empty() {
        return Err(NameError::TenantRequired);
    }
    ...
    name.push_str(&sanitise_tenant(tenant));
    name.push('-');
