...
These rules are for the Vue app in `frontend/`. The server-rendered Topcoat
UI trial in `crates/fc-web` has its own rules (see "fc-web (Topcoat UI trial)"
below) and *does* use Tailwind.

**Tailwind is not installed in `frontend/`** (it was kept out so it wouldn't
be layered on PrimeVue). Don't write Tailwind utility classes
...
- **Look = the Vue app.**
  ...
  - Before adding a page, open the matching `.vue` page and copy its values.
...
- **After changing Tailwind classes, re-bundle**:
